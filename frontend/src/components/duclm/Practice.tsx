...
const Practice: React.FC = () => {
    ...
    const fetchCurrentLevel = async () => {
      try {
        const response = await API.get(
          `http://localhost:8080/api/v1/practice/get-practice-info/${user.data?.id}`
        );
  ...
  useEffect(() => {
    const checkCompletion = async () => {
      if (currentLevel === maxLevel && user.data?.id) {
        try {
          const response = await API.get(
            `http://localhost:8080/api/v1/practice/latest-result/${user.data.id}`
          );
  ...
  useEffect(() => {
    const fetchMaxLevel = async () => {
      try {
        const response = await API.get(
          "http://localhost:8080/api/v1/practice/max-level",
          {
            headers: {
  ...
  const totalTimeSpent = testResults.reduce(
    (acc, result) => acc + convertToSeconds(result.timeSpent),
    0
  );

  const allCompleted = testResults.every(
    (result) => result.status === "Hoàn thành"
  );
  const scoreThreshold = (75 / 100) * 300; // 75% of 300

  const handleCompleteRound = async () => {
    try {
      await API.post("/practice/user-practice/add", {
        userId: user.data?.id,
        level: currentLevel,
      });
      toast({
        title: "Hoàn tất vòng thi thành công!",
        className: "text-white bg-green-500",
      });
    } catch (error) {
      console.error("Error completing round:", error);
      toast({
        title: "Lỗi khi hoàn tất vòng thi!",
  ...
  const handleRetryRound = async () => {
    try {
      const response = await API.delete(
        `/practice/test-results/delete/${currentLevel}`,
        {
          data: { userId: user.data?.id },
        }
      );
      toast({
        title: "Đã xóa kết quả để làm lại vòng thi!",
      ...
      toast({
        title: "Lỗi khi làm lại vòng thi!",
        className: "text-white bg-red-500",
      });
    }
    window.location.reload();
  ...
  return (
    <div className="bg-gray-50">
      ...
      <div className="px-6 py-8 max-w-7xl mx-auto">
        <div className="mb-4">
          <h3 className="text-xl font-semibold">
            Khối bạn đang dự thi: {user.data?.grade}
          </h3>
          <div className="text-sm text-gray-500">
            Số vòng thi IOE đang mở: {maxLevel}
          </div>
          <div className="text-sm text-gray-500">
            Vòng thi hiện tại của bạn:{" "}
            {currentLevel !== null ? currentLevel : "Đang tải..."}
          </div>
        </div>
          ...
          <div className="flex flex-col md:col-span-2">
            <div className="bg-yellow-100 p-4 rounded-lg mb-8">
              ...
              <ul className="list-disc pl-6 space-y-2 text-sm text-gray-700">
                <li>
                  Mỗi vòng Tự luyện gồm 3 bài thi. Bạn cần đạt ít nhất 75% số
                  điểm mỗi bài để hoàn thành.
                </li>
                <li>
                  Kết quả của một lần Tự luyện được tính bằng tổng điểm và tổng
                  thời gian làm bài của 3 bài thi.
                </li>
                <li>
                  ...
                  <ul className="list-inside list-disc">
                    ...
                    <li>
                      “Ghi lại kết quả” để hoàn thành và chuyển sang vòng tiếp
                      theo.
                    </li>
                  </ul>
                </li>
                <li>
                  Hệ thống chỉ lưu kết quả cao nhất để xét thứ hạng trong Bảng
                  xếp hạng tuần.
                </li>
              </ul>
            </div>
            ...
            <div className="bg-red-100 p-4 rounded-lg mb-8">
              ...
              <ul className="list-disc pl-6 space-y-2 text-sm text-gray-700">
                <li>
                  Đăng nhập một tài khoản trên hai máy khác nhau cùng lúc.
                </li>
                <li>
                  Đang làm bài mà tải lại trang hoặc thoát ra mà không nộp bài.
                </li>
                <li>Mở nhiều cửa sổ vào thi cùng lúc.</li>
              </ul>
              <p className="text-sm text-gray-700 mt-2">
                Vi phạm sẽ khiến hệ thống tự động thoát ra ngoài và tính một lần
                trượt.
              </p>
            </div>
                  ...
                  {testResults.map((result, index) => (
                    <tr key={index} className="bg-white hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        {convertToSeconds(result.timeSpent) || 0}
                      </td>
                    </tr>
                  ))}
            ...
            <div className="flex justify-center mt-6">
              {totalScore < scoreThreshold ? (
                ...
                <p className="text-green-500 font-bold">
                  Bạn đã hoàn tất toàn bộ vòng thi
                </p>
              ) : (
                <button
            ...
            <h3 className="text-lg font-semibold mt-4 mb-4">
              Kết quả hoàn thành các vòng thi
            </h3>
            ...
            <div className="bg-gray-200 p-4 rounded-lg">
              <div className="text-sm text-gray-700">
                ...
                <div>
                  Vòng thi tiếp theo:{" "}
                  <span className="font-medium">
                    {currentLevel !== null && maxLevel !== null
